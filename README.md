...
- calls `/api/live-scores` on the same origin for live Masters scores
- calculates each team's best 4 of 6 in the browser
- applies missed-cut penalties only once the cut is finalised
- preserves the last successful live data if a later refresh fails
...
- If no live data is available, the front end falls back to workbook seed scores.
- If live data has already loaded once and a later refresh fails, the page keeps showing the last successful live standings and marks them as stale.
